...
$(document).ready(function () {    
    ...
    $('#connect-form button#connect').click(function () {
        ...
        var obj = { url:'http://192.168.0.100:8080/onvif/device_service', user: $('input#user').val(), pass: $('input#pass').val() }
    ...
    ipcRenderer.on('device_discovered', (event, arg) => {
        ...
        var option_el = $('<option></option>');
            option_el.val('192.168.0.100');
            option_el.text('name' + ' (' + '192.168.0.100' + ')');
            $('select#device_selection').append(option_el);
    ...
    ipcRenderer.on('device_connected', (event, arg) => {
        ...
        $('div#device-info-box span#device-address').html("192.168.0.101");

        ipcRenderer.send('snapshot_update', arg);
